...
class PropertyController extends BaseController
{
   ...
   $a = $request->input('images');
//   dd($a);
//   $images = array_shift(array_slice($a, 0, 1));
  $fruit = array_shift($a);
//   dd($a);
foreach($a as $key => $aa){
    // dd($aa);
...
//  dd($a);
 ...
 $fruit = array_shift($a);
          ...
          $prop = PropertyModel::where('id',$property->id)->first();
          
        //   dd($prop[0]);
          $property->author_id = $prop->author_id;
 
        $property->author_type = Account::class;
        $property->images = json_encode($a);
        $property->moderation_status = $request->input('moderation_status');
        $property->never_expired = $request->input('never_expired');
