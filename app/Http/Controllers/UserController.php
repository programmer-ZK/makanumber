...
class UserController extends Controller
{
  ...
  public function activation(Request $request, $token)
  {
    ...
    if ($activations_id > 0) {
      ...
      return redirect('/admin/login')->with('status', `Dear {{$name}}, Your account has already been activated`);
    } else {
      ...
      return redirect('/admin/login')->with('status', `Dear {{$name}}, Your account is successfully activated, Please Log in to continue!`);
    }
  }
